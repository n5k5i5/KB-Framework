# -*- coding: utf-8 -*-
"""Domain WHOIS/RDAP bilgileri toplama (çekirdek)."""
...
# İsteğe bağlı: sınıf tabanlı çalıştırma desteği
...
class KB_DomainWhois(KB_Modul_Arayuzu):
    ...
    def calistir(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not requests:
            return {"durum": "hata", "mesaj": "requests kütüphanesi yüklü değil", "veri": {}}
        ...
        try:
            ...
            r.raise_for_status()
            data = r.json()
            parsed = self._parse_rdap(data)
            return {"durum": "basarili", "mesaj": "WHOIS/RDAP verisi alındı", "veri": parsed}
