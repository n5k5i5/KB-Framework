...
MANIFEST = {
    ...
    "versiyon": "0.1.0",
    "yazar": "KB-OSINT Team",
    "aciklama": "Domain için A, AAAA, MX, NS, TXT, SOA DNS kayıtlarını toplar.",
    "kategori": "domain",
    "api_gereksinimleri": [],
...
class KB_DomainDNS(KB_Modul_Arayuzu):
    ...
    def calistir(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for rtype in RECORD_TYPES:
            try:
                ...
                out["records"][rtype] = self._format_records(rtype, answers)
    ...
    def _format_records(self, rtype: str, answers) -> Dict[str, Any]:
        items: List[str] = []
        if rtype in {"A", "AAAA"}:
            for r in answers:
            ...
            for r in answers:
                # r.strings may be bytes; normalize to str
                txts = []
                try:
                    for s in r.strings:
